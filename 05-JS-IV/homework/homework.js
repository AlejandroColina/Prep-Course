...
function invocarMetodo(objeto, metodo) {
  // "metodo" es una cadena que contiene el nombre de una (funcion) en el objeto
  // Invoca ese método
...
function tienePropiedad(objeto, propiedad) {
  ...
  // Tu código:
  let bandera = Object.keys(objeto)
  if (bandera.includes(propiedad)) {
    let a = true
    return a
  }else{
    let a =false
...
function sumarLikesDeUsuario(usuario) {
  ...
  for (let i = 0; i < usuario.posts.length; i++) {
    bandera = bandera + usuario.posts[i].likes   
  }
  return bandera
}
